...
def fetch_video_list(url, max_count=25, outfile="video_list.json"):
    options = uc.ChromeOptions()
    options.add_argument('--disable-gpu')
    options.add_argument('--no-sandbox')
    driver = uc.Chrome(options=options)
    driver.get(url)
    print("[crawler] open main page, wait 5 sec...")
    time.sleep(5)
    ...
    print(f'[crawler] wrote {len(video_list)} items to {outfile}')
    return video_list
    ...
    try:
        ...
        if m3u8_url:
            try:
                ...
                print(f'[error] read m3u8 key failed: {err}')
    except Exception as e:

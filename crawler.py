...
def fetch_video_list(url, max_count=25):
    ...
    driver.get(url)
    print('[debug] opened main page, wait for 5 seconds')
    time.sleep(5)
    ...
    for item in video_items[:max_count]:
        try:
            ...
            video_list.append({
                ...
                'm3u8_url': video_res.get('m3u8_url'),
    ...
    try:
        ...
        if m3u8_url:
            try:
                ...
                print(f'[error] reading m3u8 key failed: {err}')
    except Exception as e:
...
if __name__ == '__main__':
    ...
    output_file = sys.argv[1] if len(sys.argv) > 1 else 'video_list.json'
    videos = fetch_video_list(target_url)
